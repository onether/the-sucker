window.onload = function ()
{
	...
	app.init_canvas = function (id, create, w, h, save, dom)
	{
		if (create)
		{
		...
		app.cv[id].ctx.imageSmoothingEnabled = false;
		app.cv[id].ctx.mozImageSmoothingEnabled = false;
		app.cv[id].ctx.webkitImageSmoothingEnabled = false;
	}
	...
	app.Coll_Obj.prototype.proj_out_y = function (pad, pad_id)
	{
		...
		if (dy > 0)
		{
			...
			if (this.type === 'bot' && ! this.can_move && app.plr.is_grabbing.id !== this.id)
			{
				if (this.dy > 26)
				{
					this.special_kill(0);
	...
	app.Coll_Obj.prototype.jump_sequence = function ()
	{
		if (this.type === 'player')
		{
			this.start_jump();
	...
	app.Coll_Obj.prototype.on_move = function (dir)
	{
		var p = this;
	...
	app.Coll_Obj.prototype.apply_friction_and_gravity = function ()
	{	
		if (this.pad_coll === false)
		{
	...
	app.Coll_Obj.prototype.shoot_particles = function (obj_name, x, y, dir, dir_d, f, count, count_delta, c)
	{
		var obj;
	...
	app.Plr = function () 
	{
		...
		this.hp = 30;
		this.dhp = 0;
		this.anim = 'player_run_frenzy_white';
		this.anim_frame = 0;
		this.anim_rate = 3;
		this.anim_interval = 0;
		this.jump = false;
	...
	{
		if (this.grab === true && this.grab_reload === false)
		{
			if ( ! this.is_grabbing && this.bot_0)
			{
				this.bot_0.can_move = false;
				this.is_grabbing = this.bot_0;
				app.toggle_bool('grab_reload', 300, true, this);
				app.play_sfx('grab');
			} else if (this.is_grabbing)
			{
				this.is_grabbing = false;
				this.throw_item(this.bot_0);
				app.toggle_bool('grab_reload', 300, true, this);
				app.play_sfx('grab');
			} else if ( ! this.bot_0 && ! this.frenzy)
			{
		...
		if (this.is_grabbing)
		{
			this.bot_0.x = this.x + (this.dir === 0 ? 1 : -1);
			this.bot_0.y = this.y - this.h / 2 - 6;
			this.bot_0.dx = this.dx;
			this.bot_0.dy = this.dy;
			this.bot_0.dir = this.dir;
		}
	}
	...
	app.Plr.prototype.attack_sequence = function ()
	{
		if (this.attack && this.bot_0)
		{
			this.bot_0.dhp -= this.frenzy ? 20 : 3.5;
			if (this.bot_0.hp - this.bot_0.dhp < 0 && this.is_grabbing)
			{
				delete this.is_grabbing;
			}
			
			this.dhp += this.frenzy ? 0 : (this.bot_0.bot_type !== 0 ? 0.5 : 1) * 0.45;
	...
	app.Plr.prototype.update_kill_count = function ()
	{
		this.kill_count += 1;
		//this.bot_coll = false;
		this.is_grabbing = false;
	...
	app.Plr.prototype.update_health = function ()
	{
		var bot;
		...
		if (this.frenzy)
		{
			...
			this.shoot_particles('Blood', this.x, this.y, Math.PI / 4, Math.PI / 4, 40, 5, 10);
		} else
		{
			if (this.hp > 100)
			{
	...
	{
		app.ckill_count += 1;
		app.ckill_pts_buff += (10 + 5 * bot.bot_type) * (1 + app.wave / 5) * app.ckill_count;
		...
		app.plr.score += (10 + 5 * bot.bot_type) * (1 + app.wave / 5) * app.ckill_count;
	...
	app.Bot.prototype.on_destroy = function ()
	{
		...
		if ( ! app.plr.frenzy)
		{
			app.plr.dhp += (-this.dhp) / 4.5 + (4 * this.bot_type);
		}
		
		this.shoot_particles('Blood', this.x, this.y, 0, Math.PI * 2, 30, app.plr.frenzy ? 200 : 100, 100);
		this.shoot_particles('Body_Part', this.x - this.dx, this.y - this.dy, 0, Math.PI * 2, 30, 8, 3, function (obj) 
		{
			var type = '_normal';
			switch (self.bot_type)
			{
				case 1:
				{
					type = '_green_1';
			...
			obj.anim = 'body_part_' + Math.ceil(Math.random() * 3) + type;
		...
		app.bot_track.kill[this.bot_type] += 1;
		app.bot_track.alive[this.bot_type] -= 1;
		app.bot_track.alive_count -= 1;
	...
	app.Body_Part = function ()
	{
		...
		this.timer_events = [
			[45, function ()
			{
				self.shoot_particles('Blood', self.x, self.y, 0, Math.PI * 2, 10, 1, 1);
			}]
		];
		this.destroy_timeout = setTimeout(function ()
		{
	...
	app.init_frenzy_mode_assets = function ()
	{
	...
	app.spawn_bot = function (bot_type)
	{
		var bot = app.create_obj('Bot');
		var dir = Math.random() > 0.5 ? 0 : 1;
				
		bot.bot_type = bot_type || 0;
		bot.dir = dir;
		...
		switch(bot.bot_type)
		{
			case 0:
			{
				bot.anim = 'bot_walk_normal';
				bot.dmg = 0;
				break;
			}
			case 1:
			{
				bot.anim = 'bot_walk_green_1';
				bot.hp += 25;
				bot.dmg = 0.37;
				bot.dx_max = bot.dx_max * 1.05;
				break;
			}
			case 2:
			{
				bot.anim = 'bot_walk_red_1';
				bot.hp += 50;
				bot.dmg = 0.8;
				bot.dx_max = bot.dx_max * 1.1;
				break;
			}
			case 3:
			{
				bot.anim = 'bot_walk_blue_1';
				bot.hp += 75;
				bot.dmg = 1.3;
				bot.dx_max = bot.dx_max * 1.3;
				break;
			}
		}
		
		app.bot_track.spawn[bot.bot_type] += 1;
		app.bot_track.alive[bot.bot_type] += 1;
		app.bot_track.alive_count += 1;
	}
	...
	app.update_player = function ()
	{
		...
		if (p.left || p.right)
		{
			p.anim = 'player_run_frenzy_white';
		} else
		{
			p.anim = 'player_idle_frenzy_white';
		}
	...
	app.action_mngr = function ()
	{
		...
		if ( ! app.can_spawn || app.wave_break || app.bot_track.alive_count >= app.max_bot_count + (app.wave_timer / 10))
		{
			return false;
		}
	...
	app.update = function ()
	{
		...
		switch (app.game_state)
		{
			...
			case 1:
			{
				...
				{
					if (bot.can_move || app.plr.is_grabbing === false || app.plr.is_grabbing !== bot.id)
					{
						bot.move();
						bot.level_pads_coll();
						bot.level_bounds_coll();
						bot.apply_friction_and_gravity();
						bot.jump_sequence();
						bot.update_pos();
					...
					if (bot.dhp < 0)
					{
						bot.shoot_particles('Blood', bot.x, bot.y, 0, Math.PI * 2, 15, -bot.dhp * (app.plr.frenzy ? 2 : 4), -bot.dhp);
					}
				...
				{
					elem.apply_friction_and_gravity();
					elem.update_pos();
				});
				...
				if (app.shake)
				{
					app.canvases.style.top = app.shake_y + 'px';
					app.canvases.style.left = app.shake_x + 'px';
				}
				...
				if (app.plr.left || app.plr.right)
				{
					//app.play_sfx('footstep');
				}
				break;
			}
			case 2:
	...
	app.reset_game_settings = function ()
	{
		app.timer = 0;
		app.wave = 0;
		app.wave_timer = 0;
	...
	app.draw_start_screen = function (ctx)
	{
		...
		app.draw_text(ctx, 'press f to start', app.w / 2, 0, 2, 2, true);
	...
	{	
		...
		switch (app.game_state)
		{
			...
			case 0:
			{
				app.draw_text(ctx, app.intro_msg[0] + ' (f)', app.w / 2, app.h / 2 - 4 * 6, 2, 2, true);
				app.draw_noise(app.cv.noise.ctx);
				break;
			}
			case 1:
			{
				...
				{
					p.anim = p.anim.split('_frenzy_')[0] + ('_frenzy_' + (Math.random() > 0.5 ? 'white' : 'red'));
				} else
				{
					p.anim = p.anim.split('_frenzy_')[0] + '_frenzy_white';
				}
			...
			case 2:
			{
				...
				ctx.fillStyle = '#fff';
				//ctx.fillText(app.mx + ':' + app.my, app.mx, app.my);
				app.draw_noise(app.cv.noise.ctx);
				
				if (app.menu_hover_y)
				{
					ctx.fillStyle = 'rgba(255,0,0,' + (0.2 + Math.random() * 0.4) + ')';
					ctx.fillRect(app.menu_hover_x * (app.w / 2), app.menu_hover_y, app.w / 2, 60);
				}
	...
	app.draw_summary = function (ctx)
	{
		...
		//app.draw_text(ctx, 'tweet your score', 0, 360, 2, 2, true);
	...
	app.start_game = function ()
	{
		//app.intro_guide();
		//app.start_wave();
		//app.toggle_bool('wave_break', 10000, true);
	}
	...
	app.end_game = function ()
	{
		...
		if (r === 1)
		{
			...
			{
				p.name = prompt('Congratulations! New best local score! Please enter your name');
				...
				app.save_cfg('sucker_h_scores', app.h_scores);
			}, 50);
		}
	...
	app.load_sfx = function ()
	{
		...
		{
			var audio_data = app.gen_sfx_data(app.sfx_data[k].d, 100, 100, app.sfx_data[k].fn);
			app.sfx[k] = app.gen_audio_elem(audio_data);
			//app.audio.play();
		};
	}
	
	app.img_from_data = function (data, w_scale, h_scale)
	{
		var cv;
		var w;
		var h;
		var name = data[0];
		var subtype_name = data[2];
		var img_data = data[1].split(' ');
		var subtypes = subtype_name ? app.gfx_subtypes[subtype_name] : { 'default': 'default' };
		! w_scale ? w_scale = 1 : '';
		! h_scale ? h_scale = 1 : '';
		
		w = img_data[0].length * 6 * w_scale;
		h = img_data.length * 6 * h_scale;
		
		for (var k in subtypes)
		{
			cv = app.create_cv(name + (k !== 'default' ? '_' + k : ''), w, h, true);
			
			img_data.forEach(function (r, r_id)
			{
				var l = r.length;
				
				for (var a = 0; a < l; a += 1)
				{
					cv.ctx.fillStyle = k !== 'default' ? (img_data[r_id][a] === 'e' ? subtypes[k] : app.gfx_pal[img_data[r_id][a]]) : app.gfx_pal[img_data[r_id][a]];
					cv.ctx.fillRect(a * 6 * w_scale, r_id * 6 * h_scale, 6 * w_scale, 6 * h_scale);
				}
			});
		}
	}
	
	app.img_roman_big = function ()
	{
	...
	app.load_gfx = function ()
	{
		app.gfx_subtypes = {
			player: {
				frenzy_white: 'rgba(255,255,255,1)', 
				frenzy_red: 'rgba(255,0,0,1)'
			},
			bot: {
				normal: 'rgba(141,128,101,1)',
	...
	app.init_keys = function ()
	{
		...
		{
			if ( ! app.key_pressed || app.key_pressed === false)
			{
				switch (evt.keyCode)
				{
					case 38:
		...
		window.onmousemove = function (evt)
		{
			...
			if (app.game_state === 2)
			{
				...
				{
					app.menu_hover_y = 624;
					app.menu_hover_x = Math.floor(app.mx / (app.w / 2));
				}
			}
		}
			...
			if (app.game_state === 2)
			{
				if(app.my > 624 && app.my < 684)
				{
					if (app.mx < app.w / 2)
					{
						//restart game
						app.game_state = 1;
						app.reset_game_settings();
						app.on_wave_start();
						app.cv.gore.ctx.clearRect(0,0,app.w,app.h);
					} else 
					{
						var l = window.location.href;
						var url = 'https://twitter.com/intent/tweet?hashtags=js13kGames&original_referer=' + l +'&source=tweetbutton&text=I%20scored%20' + (app.plr.score) + '%20points%20over%20' + (app.wave + 1) +'%20wave' + (app.wave > 0 ? 's ' : '') + '%20in%20%22The%20Sucker%22%20game%2C%20a%20js13kGames%20entry%20-%20&url=http%3A%2F%2Fether-wp.com%2Fdistillery%2Fsucker%2F&via=pordesign'
						var name = 'Tweet your The Sucker achievement!';
						
						var w = window.open(url, name, 'height=600,width=800');
					}
				}
			}
			
			//app.plr.shoot_particles('Blood', app.w / 2, app.h / 2, Math.PI / 4, Math.PI / 4, 30, 100, 100);
		}
	}
	...
	app.init_html_menu = function ()
	{
		...
		var blood_levels = 9;
		var blood_level_select = document.m.blood_level;
		
		for (var a = 0; a <= blood_levels; a += 1)
		{
			blood_level_select.options[a] = new Option(a + (a === 3 ? ' (default)' : ''), a, false, a === app.url_params.blood_level ? true : false);
		}
		
		options.forEach(function (elem)
		{
			var html_elem = document.getElementById('toggle_' + elem[0])
			html_elem.onclick = function ()
			{
				
				if (app.game_state === 1)
				{
					if ( ! confirm('This will reset current game. Are you sure?'))
					{
						return false;
					}
				}
				app.url_params[elem[0]] = app.url_params[elem[0]] ? 0 : 1;
				html_elem.innerHTML = elem[1] + ' ' + (app.url_params[elem[0]] == true ? 'on' : 'off');
				app.update_url();
			}
		...
		blood_level_select.onchange = function ()
		{
			app.url_params.blood_level = blood_level_select.selectedIndex;
			app.update_url();
		}
	}
	...
	app.update_url_params = function ()
	{
		...
		if (window.location.href.indexOf('?') !== -1)
		{
			str = window.location.href.split('?')[1].split('&').forEach(function (elem)
			{
				...
				if (app.url_params[param[0]])
				{
					app.url_params[param[0]] = param[1] * 1 == param[1] ? param[1] * 1 : param[1];
				}
			});
		}
	}
	...
	app.init_app = function ()
	{
		...
		{	
			...
			app.load_content_interval = setInterval(function ()
			{
				if (app.content_loaded())
				{
					app.init_app();
					app.close_overlay();
					clearInterval(app.load_content_interval);
				};
			}, 50);	
		...
		} else
		{
			...
			{
				app.init_canvas(elem, true, app.w, app.h, true, true);
			});
			app.canvases = document.getElementById('cvs');
			...
			app.h_scores = app.load_cfg('sucker_h_scores') || [];
